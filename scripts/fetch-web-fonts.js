...
async function removeLegacyFonts() {
...
async function fetchTtfFromRelease() {
    const fontsDir = path.join(root, "app", "fonts");
    fs.mkdirSync(fontsDir, { recursive: true });
    await removeLegacyFonts();
    const missing = TTF_ASSETS.filter((asset) => !fs.existsSync(asset.dest) || fs.statSync(asset.dest).size < 1000);
    if (missing.length === 0) {
...
async function fetchWoff2Assets() {
