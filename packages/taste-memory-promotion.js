"use strict";
const _tmDeps = typeof module !== "undefined" && module.exports
...
const { normalizePrinciple: normalizePrincipleBase, tasteGenId: tasteGenIdFn, PRINCIPLE_POLARITIES: principlePolarities, } = _tmDeps;
const PROMOTE_PROJECT_THRESHOLD = 2;
function promotionIsRecord(value) {
    return Boolean(value) && typeof value === "object" && !Array.isArray(value);
}
function promotionStringList(value) {
...
function normalizeGlobalPrinciple(p) {
    ...
    return {
        ...base,
        evidenceCount: Math.max(projectIds.length, Number(p.evidenceCount) || projectIds.length || 1),
        projectIds,
        lastReinforced: typeof p.lastReinforced === "string"
            ? p.lastReinforced
...
function globalTasteSummary(global) {
    ...
    return {
        ...
        promotionCandidates: g.pending.filter((p) => p.projectIds.length >= PROMOTE_PROJECT_THRESHOLD),
    };
}
/**
...
function promoteStatementToGlobal(global, input, projectId) {
    ...
    if (!statement)
        return { global: g, ok: false, error: "statement required" };
    const key = statementKey(statement);
    const existing = g.principles.find((x) => statementKey(x.statement) === key);
    if (existing) {
