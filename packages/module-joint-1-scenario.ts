...
type MJPoint = [number, number];
type MJRing = MJPoint[];
type MJPolygon = MJRing[];

interface MJHoleCenter {
  id: string;
  cx: number;
  cy: number;
}
...
declare function replaceState(state: any): void;
declare function render(): void;
declare function uiUpdate(): void;
...
function moduleJoint1R(mm: number): number {
  return mm * MODULE_JOINT_1_REAL_PER_MM;
}

function moduleJoint1Layout(): { ox: number; oy: number; w: number; h: number } {
...
function moduleJoint1CircleRing(
  cx: number,
  cy: number,
  r: number,
  segments = 128,
): MJRing {
  return Array.from({ length: segments }, (_, i): MJPoint => {
    const a = (2 * Math.PI * i) / segments;
    return [cx + r * Math.cos(a), cy + r * Math.sin(a)];
  });
}

function moduleJoint1OutlineRing(): MJRing {
...
function moduleJoint1HoleCenters(): MJHoleCenter[] {
  ...
  return Array.from({ length: MODULE_JOINT_1_MM.rows }, (_, i) => {
    ...
    return xs.map(
      (x, col): MJHoleCenter => ({ id: `r${i + 1}c${col + 1}`, cx: x, cy: y }),
    );
  }).flat();
}

// polygon-clipping: ブラウザはグローバル、node は vendor UMD を require。
function moduleJoint1PolyClip(): any {
...
function moduleJoint1PathShape(id = "module-joint-1-outline"): MillrectShape {
  ...
  if (pc && slots.length) {
    try {
      ...
      if (diff && diff.length) {
        const stripClose = (ring: MJRing): MJRing => {
          const n = ring.length;
          if (
            n > 1 &&
            ring[0][0] === ring[n - 1][0] &&
        ...
        contours = diff.map((poly: MJRing[]) => poly.map(stripClose));
      }
    } catch (e) {
      /* フォールバック: 焼き込みなし（穴のみ） */
    }
...
function moduleJoint1DimensionShapes(): any[] {
  const { ox, oy, w, h } = moduleJoint1Layout();
