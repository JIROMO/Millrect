...
function _pickFontFileUrlFromCss(css) {
    const urls = [...css.matchAll(/url\((https:[^)]+)\)/g)].map((m) => m[1].replace(/['"]/g, ""));
    if (!urls.length) {
    ...
    if (woff2.length === 1)
        return woff2[0];
    if (woff2.length > 1) {
        throw new Error("CSS が subset 分割のみです。Fontsource 経由で解決してください");
    }
    return urls[urls.length - 1];
}
async function resolveFontsourceFileUrl(family, weight = 400) {
    const slug = family.toLowerCase().trim().replace(/\s+/g, "-");
    const metaRes = await fetch(`https://api.fontsource.org/v1/fonts/${encodeURIComponent(slug)}`);
    if (!metaRes.ok) {
