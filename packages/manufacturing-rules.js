...
function _manufacturingNum(v, fallback) {
    const n = Number(v);
...
function normalizeManufacturing(raw = {}) {
    const m = raw && typeof raw === "object" && !Array.isArray(raw)
        ...
        : {};
    return {
        process: typeof m.process === "string" ? m.process : DEFAULT_MANUFACTURING.process,
        thickness_mm: _manufacturingNum(m.thickness_mm ?? m.thicknessMm, DEFAULT_MANUFACTURING.thickness_mm),
        kerf_mm: _manufacturingNum(m.kerf_mm ?? m.kerfMm, DEFAULT_MANUFACTURING.kerf_mm),
        min_hole_diameter_mm: _manufacturingNum(m.min_hole_diameter_mm ?? m.minHoleDiameterMm, DEFAULT_MANUFACTURING.min_hole_diameter_mm),
        min_edge_distance_mm: _manufacturingNum(m.min_edge_distance_mm ?? m.minEdgeDistanceMm, DEFAULT_MANUFACTURING.min_edge_distance_mm),
    };
}
...
function validateManufacturingRules(dsl = {}) {
    const manufacturing = normalizeManufacturing(dsl.manufacturing ?? dsl.meta?.manufacturing);
    const violations = [];
    ...
    for (const spec of specs) {
        if (spec.kind === "hole") {
            ...
            if (d < manufacturing.thickness_mm &&
