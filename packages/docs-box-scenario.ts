...
type DocsBoxPoint = [number, number];
type DocsBoxRing = DocsBoxPoint[];

interface DocsBoxScale {
  numerator: number;
  denominator: number;
}
...
function docBoxTopLayout(
  mm: DocsBoxMm = DOC_DOCS_MM,
  scale: DocsBoxScale = DOC_DOCS_SCALE,
) {
...
function docCircleRing(
  ...
  return Array.from({ length: segments }, (_, i) => {
    ...
    return [cx + r * Math.cos(a), cy + r * Math.sin(a)] as DocsBoxPoint;
  });
}

function docBoxTopFeatureLayout(
  mm: DocsBoxMm = DOC_DOCS_MM,
  scale: DocsBoxScale = DOC_DOCS_SCALE,
) {
...
function docBoxTopPathShape(id: string, style: DocsBoxStyle = DOC_DOCS_BOX.top) {
