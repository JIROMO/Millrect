...
function countNegativeRings(contours) {
    ...
    for (const poly of contours || []) {
        for (const ring of poly || []) {
            if (ring?.length > 2 && ringSignedArea(ring) < 0)
                count++;
        }
    }
    return count;
...
function glyphFillRule(contours) {
    if (countNegativeRings(contours) > 0)
        return "nonzero";
    const rings = flattenRings(contours);
    if (shouldUnionOverlappingPositiveRings(rings))
        return "nonzero";
    return "evenodd";
}
...
function shouldUnionOverlappingPositiveRings(rings) {
    if (rings.length <= 1)
        return false;
    if (rings.some((r) => ringSignedArea(r) < 0))
        return false;
    for (let i = 0; i < rings.length; i++) {
        const [cx, cy] = ringCenter(rings[i]);
        for (let j = 0; j < rings.length; j++) {
