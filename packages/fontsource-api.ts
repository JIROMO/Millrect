...
interface FontsourceTtfOpts {
  weight?: number;
  style?: string;
  preferJapanese?: boolean;
}

interface ProjectFontUrls {
  fileUrl: string;
  fileUrlBold: string | null;
  cssUrl: string;
}

interface FontCatalogFilterOpts {
  query?: string;
...
function buildGoogleFontsCssUrl(family: unknown, weights?: number[]): string {
...
function fontsourceSlug(familyOrId: unknown): string {
...
function pickFontsourceTtfUrl(meta: any, opts: FontsourceTtfOpts = {}): string {
  ...
  if (variants && typeof variants === "object") {
    ...
    if (styleObj && typeof styleObj === "object") {
      const subsets: string[] = meta.subsets || [];
...
async function fetchFontsourceCatalog(query = ""): Promise<any[]> {
...
function filterFontCatalog(items: any[], opts: FontCatalogFilterOpts = {}): any[] {
