...
function buildGoogleFontsCssUrl(family, weights) {
    ...
    if (!name)
        throw new Error("フォント名が空です");
    const param = name.replace(/\s+/g, "+");
    const w = Array.isArray(weights)
        ? [...new Set(weights.filter((n) => Number.isFinite(n)))].sort((a, b) => a - b)
        : [];
...
function pickFontsourceTtfUrl(meta, opts = {}) {
    ...
    if (variants && typeof variants === "object") {
        ...
        if (styleObj && typeof styleObj === "object") {
            ...
            if (subsets.includes("latin"))
                subsetOrder.push("latin");
            for (const sub of subsets) {
                if (!subsetOrder.includes(sub))
                    subsetOrder.push(sub);
            }
    ...
    const subset = preferJapanese && subsets.includes("japanese")
        ? "japanese"
...
async function fetchFontsourceCatalog(query = "") {
    ...
    if (!Array.isArray(data))
        throw new Error("Fontsource 一覧の形式が不正です");
    return data;
}
async function fetchFontsourceFontMeta(idOrFamily) {
    ...
    if (!res.ok) {
        throw new Error(`Fontsource に「${idOrFamily}」が見つかりません (${res.status})`);
    }
    return res.json();
}
function filterFontCatalog(items, opts = {}) {
    ...
    if (category) {
        out = out.filter((item) => String(item.category || "").toLowerCase() === category);
    }
    if (q) {
        out = out.filter((item) => {
