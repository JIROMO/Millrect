...
type TasteRecord = Record<string, any>;

function tasteIsRecord(value: unknown): value is TasteRecord {
  return Boolean(value) && typeof value === "object" && !Array.isArray(value);
}

function tasteGenId(prefix: string): string {
  return (
    prefix +
    "-" +
...
function createEmptyProjectBrief(overrides?: unknown): TasteRecord | null {
  const base = {
  ...
  if (!tasteIsRecord(overrides)) return base;
  return mergeProjectBrief(null, overrides);
}

function normalizeTasteRef(ref: unknown): TasteRecord | null {
  if (!tasteIsRecord(ref)) return null;
  const type = ref.type;
...
function normalizePrinciple(p: unknown): TasteRecord | null {
  if (!tasteIsRecord(p)) return null;
  ...
  return {
    ...
    sources: Array.isArray(p.sources)
      ? p.sources.filter((s: unknown) => typeof s === "string")
      : undefined,
    promoteBlocked: p.promoteBlocked === true ? true : undefined,
  };
}

function normalizeJudgment(j: unknown): TasteRecord | null {
  if (!tasteIsRecord(j)) return null;
...
function normalizeArtifactEntry(e: unknown): TasteRecord | null {
  if (!tasteIsRecord(e)) return null;
  ...
  return {
    ...
    evaluation: tasteIsRecord(e.evaluation)
      ? {
          aligned: Array.isArray(e.evaluation.aligned)
            ? e.evaluation.aligned.filter((x: unknown) => typeof x === "string")
            : undefined,
          misaligned: Array.isArray(e.evaluation.misaligned)
            ? e.evaluation.misaligned.filter(
    ...
    linkedJudgmentIds: Array.isArray(e.linkedJudgmentIds)
      ? e.linkedJudgmentIds.filter((x: unknown) => typeof x === "string")
      : undefined,
  };
}

function normalizeProjectBrief(raw: unknown): TasteRecord | null {
  if (raw == null) return null;
  if (!tasteIsRecord(raw)) return null;
...
function isEmptyProjectBrief(brief: unknown): boolean {
  if (!tasteIsRecord(brief)) return true;
  return (
    !brief.intent &&
...
function mergeProjectBrief(
  existing: unknown,
  patch?: unknown,
): TasteRecord | null {
  const base = normalizeProjectBrief(existing) || createEmptyProjectBrief();
  if (!tasteIsRecord(base)) return null;
  if (!tasteIsRecord(patch)) return base;
...
function evaluateBriefBeforeMake(brief: TasteRecord | null, required: boolean) {
...
function briefSummary(brief: TasteRecord | null) {
  ...
  return {
    ...
    recentDecisions: decisions.slice(-3).map((d: TasteRecord) => ({
      id: d.id,
      outcome: d.outcome,
      reason: d.reason,
      at: d.at,
