...
function normalizePanelParams(params = {}) {
    return {
        ...
        H: _num(params.H ?? params.height ?? params.h ?? params.D ?? params.depth, DEFAULT_PANEL_PARAMS.H),
    };
}
function normalizeLBracketParams(params = {}) {
    return {
        ...
        T: _num(params.T ?? params.t ?? params.thickness, DEFAULT_L_BRACKET_PARAMS.T),
...
function normalizeEnclosureParams(params = {}) {
    ...
    return {
        ...box,
        T: _num(params.T ?? params.t ?? params.thickness, DEFAULT_ENCLOSURE_PARAMS.T),
    };
}
...
function normalizePartDsl(raw = {}) {
    ...
    const manufacturing = (_normalizeManufacturing ?? normalizeManufacturing)(raw.manufacturing ?? raw.meta?.manufacturing);
    return {
        ok: true,
        dsl: {
            version: PART_DSL_VERSION,
