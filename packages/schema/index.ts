...
(function initSchema(root: any, factory: () => Record<string, any>) {
  const api = factory();
  ...
  function factory() {
    ...
    function getPaperSizeMm(page: Partial<MillrectPage> = {}): {
      width: number;
      height: number;
    } {
      const size = PAPER_SIZES_MM[page.paper as string] || PAPER_SIZES_MM.A4;
      return page.orientation === "landscape"
        ? { width: size.height, height: size.width }
        : { width: size.width, height: size.height };
    }

    function getPageCanvasSizeMm(page: Partial<MillrectPage> = {}): {
      width: number;
      height: number;
    } {
    ...
    function validateModelIr(ir: any): MillrectValidationResult {
      ...
      if (!ir.operations?.some((operation: any) => operation.type === "extrude")) {
