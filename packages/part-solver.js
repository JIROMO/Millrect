...
function _deps() {
    ...
    return {
        ...
        boxViewSizesMm: typeof window !== "undefined" ? window.boxViewSizesMm : null,
        normalizeViewType: typeof window !== "undefined"
            ? window.normalizeViewType
            : (t) => t,
    };
}
...
function _viewParamMap(partKind) {
    if (partKind === "panel")
        return PANEL_VIEW_PARAM_MAP;
    if (partKind === "l_bracket")
        return L_BRACKET_VIEW_PARAM_MAP;
...
function _partGeometry() {
    ...
    return {
        lBracketTopOuterRing: typeof window !== "undefined"
            ? window.lBracketTopOuterRing
            : null,
        centerRingsOnPaper: typeof window !== "undefined" ? window.centerRingsOnPaper : null,
    };
}
/**
...
function targetSizesReal(partKind, params) {
    ...
    for (const [view, s] of Object.entries(mm)) {
        out[view] = {
            width: _partMmToReal(s.w),
            height: _partMmToReal(s.h),
        };
    }
    return out;
}
function _rebuildLBracketProfiles(state, partIntent, helpers) {
    ...
    for (const [view, binding] of Object.entries(partIntent.bindings)) {
        ...
        if (view === "top" && shape.type === "path") {
            const paperMm = typeof getPaperDimensions === "function"
            ...
            resizeRectCentered(shape, _partMmToReal(params.A), _partMmToReal(params.H));
            updated.push({ view, shapeId: shape.id, type: "rect" });
        }
    }
    partIntent.paramConstraints = syncParamBindConstraints(partIntent.paramConstraints, params);
...
function applyParamBindSolver(state, partIntent, helpers = {}) {
    ...
    for (const [view, binding] of Object.entries(partIntent.bindings)) {
        ...
        if (shape.type === "rect") {
            ...
            layer.shapes[idx] = profileShapeToRect(shape, target.width, target.height, bb);
...
function buildPartIntentRecord(dsl, compiled, state, buildOptions) {
    ...
    return {
        ...
        paramConstraints: syncParamBindConstraints((compiled.constraints || []).filter((c) => c.kind === "param_bind"), compiled.dsl.params),
        geometricConstraints: (compiled.constraints || []).filter((c) => c.kind !== "param_bind"),
    };
}
