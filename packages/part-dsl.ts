...
type PartDslRecord = Record<string, any>;

interface FeatureNormalizeResult {
  ok: boolean;
  error?: string;
  feature?: PartDslRecord;
}

interface PartDslNormalizeResult {
  ok: boolean;
  error?: string;
  dsl?: PartDslRecord;
}
...
let _normalizeManufacturing: ((m: unknown) => any) | undefined;
let _validateManufacturingRules: ((dsl: unknown) => any) | undefined;
...
function legacyOptionsToPartDsl(options: PartDslRecord = {}): PartDslRecord {
  const size = options.sizeMm ?? options.params ?? {};
  const part = options.kind ?? options.part ?? "box";
  const params: PartDslRecord = { ...size };
...
function normalizePanelParams(params: PartDslRecord = {}) {
  return {
...
function normalizeLBracketParams(params: PartDslRecord = {}) {
  return {
...
function normalizeBoxParams(params: PartDslRecord = {}) {
  return {
...
function normalizeEnclosureParams(params: PartDslRecord = {}) {
...
function normalizePartParams(part: string, params: PartDslRecord = {}) {
  switch (part) {
    case "panel":
...
function defaultViewsForPart(part: string): string[] {
  switch (part) {
    case "panel":
      return ["top"];
    case "l_bracket":
...
function normalizePartDsl(raw: PartDslRecord = {}): PartDslNormalizeResult {
  ...
  const features: PartDslRecord[] = [];
  for (const f of raw.features ?? []) {
    const norm = normalizeFeature(f);
    if (!norm.ok) return norm;
    features.push(norm.feature as PartDslRecord);
  }
...
function _compileBoxParamConstraints(params: PartDslRecord): PartDslRecord[] {
  return [
    {
      id: "dsl-param-W",
      kind: "param_bind",
...
function _compilePanelParamConstraints(params: PartDslRecord): PartDslRecord[] {
  return [
    {
      id: "dsl-param-W",
      kind: "param_bind",
...
function _compileLBracketParamConstraints(
  params: PartDslRecord,
): PartDslRecord[] {
  return [
    {
      id: "dsl-param-A",
...
function _compileParamConstraints(
  part: string,
  params: PartDslRecord,
): PartDslRecord[] {
  switch (part) {
...
function _buildOptionsForPart(dsl: PartDslRecord): PartDslRecord {
...
function compilePartDsl(raw: PartDslRecord = {}): PartDslRecord {
  const normalized =
  ...
  const dsl = normalized.dsl as PartDslRecord;
