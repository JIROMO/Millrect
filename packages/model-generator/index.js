...
(function initModelGenerator(root, factory) {
...
})(typeof globalThis !== "undefined" ? globalThis : null, function factory(schema) {
    if (!schema)
        throw new Error("Millrect schema package is required");
    ...
    function roundedRectRing(x, y, w, h, tl, tr, br, bl, steps = 32) {
        const arc = (cx, cy, r, a0, a1) => Array.from({ length: steps + 1 }, (_, i) => {
            const a = a0 + ((a1 - a0) * i) / steps;
            return [round(cx + r * Math.cos(a)), round(cy + r * Math.sin(a))];
        });
        const pts = [];
        if (tl > 0)
            pts.push(...arc(x + tl, y + tl, tl, Math.PI, Math.PI * 1.5));
        else
            pts.push([x, y]);
        if (tr > 0)
    ...
    function shapeToProfileRings(shape, warnings, ancestorGroups = []) {
        ...
        switch (shape?.type) {
            case "rect": {
                ...
                if (shape.rxMode === "individual") {
                    outer = roundedRectRing(x, y, w, h, Number(shape.rxTL) || 0, Number(shape.rxTR) || 0, Number(shape.rxBR) || 0, Number(shape.rxBL) || 0);
                }
                else if (shape.rx) {
            ...
            case "circle":
                if (!(Number(shape.r) > 0))
                    return null;
                rings = [
                    circleRing(Number(shape.cx) || 0, Number(shape.cy) || 0, Number(shape.r)),
                ];
                break;
            case "ellipse":
                if (!(Number(shape.rx) > 0) || !(Number(shape.ry) > 0))
                    return null;
                rings = [
        ...
        if (hasVisualTransform(shape, ancestorGroups)) {
            warnings.push(`Profile ${shape.id || "(unknown)"} has visual transforms; Model IR keeps raw rings for now`);
        }
    ...
    function inferDimensions(profiles, warnings) {
        ...
        const width = maxProfileSize(profiles, ["front", "back"], "w") ??
            maxProfileSize(profiles, ["top", "bottom"], "w") ??
            0;
        const depth = maxProfileSize(profiles, ["top", "bottom"], "h") ??
            maxProfileSize(profiles, ["right", "left"], "w") ??
            0;
        const height = maxProfileSize(profiles, ["front", "back"], "h") ??
