...
var HELP_INDEX_EN: Record<string, HelpEntryLocalized> = {
...
function helpEntryKey(entry: HelpEntry): string {
  return entry.anchor ? entry.page + "#" + entry.anchor : entry.page;
}

function normalizeHelpLocale(locale?: string): "ja" | "en" {
  return locale === "en" ? "en" : "ja";
}

function getHelpSearchLocale(): "ja" | "en" {
...
function localizeHelpEntry(entry: HelpEntry, locale?: string): HelpEntry {
...
function searchHelpIndex(
  query: string,
  limit?: number,
  locale?: string,
): Array<HelpEntry & { score: number }> {
  limit = limit || 12;
  ...
  var scored: { entry: HelpEntry; score: number }[] = [];
...
function helpTopicHref(entry: HelpEntry): string {
  return entry.anchor ? entry.page + "#" + entry.anchor : entry.page;
}
