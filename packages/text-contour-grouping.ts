...
function ringCenter(ring: Ring): Point {
  let x = 0;
  let y = 0;
...
function normalizeRingByDepth(ring: Ring, depth: number): Ring {
...
function groupRingsIntoPolygons(rings: Ring[]): Contours {
  ...
  const children = rings.map((): number[] => []);
  for (let i = 0; i < rings.length; i++) {
    if (parent[i] >= 0) children[parent[i]].push(i);
  }

  const collect = (idx: number, depth: number, out: Polygon): void => {
...
function countNegativeRings(contours?: Contours): number {
  let count = 0;
...
function glyphFillRule(contours: Contours): "nonzero" | "evenodd" {
...
function flattenRings(contours?: Contours): Ring[] {
  const rings: Ring[] = [];
  for (const poly of contours || []) {
    for (const ring of poly || []) {
...
function shouldUnionOverlappingPositiveRings(rings: Ring[]): boolean {
...
function shouldUnionStrokeFragments(prepared?: Contours): boolean {
...
if (typeof window !== "undefined") {
  Object.assign(window, { __millrectTextContourGrouping: api });
}
