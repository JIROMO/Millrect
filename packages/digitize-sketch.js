...
function _digitizeNum(v) {
    const n = Number(v);
    return Number.isFinite(n) ? n : null;
}
function isDigitizeType(type) {
    return typeof type === "string" && DIGITIZE_TYPES.has(type);
}
/** mm 値を real units に変換（proposal は mm 第一級） */
function proposalToShape(proposal, opts = {}) {
    if (!proposal?.type || !isDigitizeType(proposal.type))
        return null;
    const id = proposal.id || (opts.genId ? opts.genId() : `digitize-${Date.now()}`);
