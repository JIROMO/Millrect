...
function _deps(): PartSolverRecord {
  ...
  return {
    REAL_PER_MM: typeof window !== "undefined" ? (window as any).REAL_PER_MM : 10,
    boxViewSizesMm:
      typeof window !== "undefined" ? (window as any).boxViewSizesMm : null,
    normalizeViewType:
      typeof window !== "undefined"
        ? (window as any).normalizeViewType
        : (t: string) => t,
  };
}

function _partMmToReal(mm: number): number {
...
const BOX_VIEW_PARAM_MAP: PartSolverRecord = {
...
const PANEL_VIEW_PARAM_MAP: PartSolverRecord = {
  top: { widthParam: "W", heightParam: "H" },
};

const L_BRACKET_VIEW_PARAM_MAP: PartSolverRecord = {
  top: { widthParam: "A", heightParam: "B", profile: "l_path" },
  front: { widthParam: "A", heightParam: "H" },
};

function _viewParamMap(partKind: string): PartSolverRecord {
...
function _partGeometry(): PartSolverRecord {
  if (typeof require !== "undefined") {
    try {
  ...
  return {
    lBracketTopOuterRing:
      typeof window !== "undefined"
        ? (window as any).lBracketTopOuterRing
        : null,
    centerRingsOnPaper:
      typeof window !== "undefined" ? (window as any).centerRingsOnPaper : null,
  };
}
 ...
 */
function syncParamBindConstraints(
  paramConstraints: PartSolverRecord[] = [],
  ...
  return (paramConstraints || []).map((c) => {
    ...
    return {
      ...c,
      value: val,
      binds: (c.binds || []).map((b: PartSolverRecord) => ({ ...b, mm: val })),
    };
  });
}
 ...
 */
function buildProfileBindings(
  pages: PartSolverRecord[] = [],
  partKind = "box",
) {
  const normalizeViewType = _deps().normalizeViewType || ((t: string) => t);
  const map = _viewParamMap(partKind);
  const bindings: PartSolverRecord = {};
...
function targetSizesReal(partKind: string, params: PartSolverRecord) {
  ...
  for (const [view, s] of Object.entries(mm)) {
    out[view] = {
      ...
      height: _partMmToReal((s as PartSolverRecord).h),
    };
  }
  return out;
}

function _rebuildLBracketProfiles(
  state: PartSolverRecord,
  partIntent: PartSolverRecord,
  helpers: PartSolverRecord,
) {
  void helpers;
  ...
  for (const [view, binding] of Object.entries(partIntent.bindings)) {
    const page = state.pages.find(
      (p: PartSolverRecord) => p.id === (binding as PartSolverRecord).pageId,
    );
    if (!page) continue;
    const found = _psFindShapeOnPage((binding as PartSolverRecord).shapeId, page);
    if (!found) continue;
...
function _psFindShapeOnPage(id: string, page: PartSolverRecord) {
  for (const layer of page.layers || []) {
    const s = layer.shapes.find((sh: PartSolverRecord) => sh.id === id);
 ...
 */
function installDslConstraints(
  state: PartSolverRecord,
  constraints: PartSolverRecord[] = [],
  bindings: PartSolverRecord = {},
  ...
  for (const c of constraints || []) {
    ...
    for (const p of state.pages) {
      if (shapeIds.every((id: string) => _psFindShapeOnPage(id, p))) {
        page = p;
        break;
      }
    }
...
function buildPartIntentRecord(
  dsl: PartSolverRecord,
  compiled: PartSolverRecord,
  state: PartSolverRecord,
  buildOptions: PartSolverRecord,
) {
  void dsl;
  return {
    version: 1,
    ...
    paramConstraints: syncParamBindConstraints(
      (compiled.constraints || []).filter(
        (c: PartSolverRecord) => c.kind === "param_bind",
      ),
      compiled.dsl.params,
    ),
