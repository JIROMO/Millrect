...
exports.outlineTextNative = outlineTextNative;
exports.measureTextLayoutNative = measureTextLayoutNative;
const fontkit = require("fontkit");
const { findFontFile } = require("./font-path");
const { BUILTIN_FONT_GEN, normalizeTextFontFamily, } = require("../packages/builtin-fonts");
const { textEngineGroupRingsIntoPolygons, } = require("../packages/text-engine-utils");
...
function openFontkitFile(filePath, familyHint) {
    ...
    if (!handle.fonts?.length)
        return handle;
    if (!familyHint)
        return handle.fonts[0];
...
function resolveFontFromPayload(shape, payload) {
    ...
    for (const pf of projectFiles) {
        if (!pf?.data?.length)
            continue;
        if (pf.bold !== undefined && pf.bold !== wantBold)
            continue;
        try {
            const font = openFontkitBuffer(Buffer.from(pf.data), pf.family);
            if (font)
                return { font, familyName: pf.family || font.familyName };
        }
        catch (_) { }
    }
    ...
    for (const f of expandFontFamilyCandidates(shape))
        add(f);
    if (textNeedsCjk(shape?.text))
        add(BUILTIN_FONT_GEN);
    for (const family of families) {
        for (const style of styles) {
            ...
            try {
                ...
                if (font)
                    return { font, familyName: font.familyName || family };
            }
            catch (_) { }
        }
    }
...
function glyphToContours(glyphPath, scale) {
    ...
    if (!rings.length)
        return null;
    const contours = textEngineGroupRingsIntoPolygons(rings).map((poly) => poly.map((ring) => ring.map(([x, y]) => paperToReal(x, y, scale))));
...
function sampleCubic(x0, y0, x1, y1, x2, y2, x3, y3, emit) {
    for (let i = 1; i <= BEZIER_STEPS; i++) {
        ...
        const mt = 1 - t;
        emit(mt ** 3 * x0 + 3 * mt ** 2 * t * x1 + 3 * mt * t ** 2 * x2 + t ** 3 * x3, mt ** 3 * y0 + 3 * mt ** 2 * t * y1 + 3 * mt * t ** 2 * y2 + t ** 3 * y3);
    }
}
function sampleQuad(x0, y0, x1, y1, x2, y2, emit) {
    for (let i = 1; i <= BEZIER_STEPS; i++) {
        const t = i / BEZIER_STEPS;
        const mt = 1 - t;
        emit(mt ** 2 * x0 + 2 * mt * t * x1 + t ** 2 * x2, mt ** 2 * y0 + 2 * mt * t * y1 + t ** 2 * y2);
    }
}
    ...
    for (const cmd of commands) {
        switch (cmd.command) {
            ...
            case "quadraticCurveTo":
                sampleQuad(cx, cy, cmd.args[0], cmd.args[1], cmd.args[2], cmd.args[3], (x, y) => ring.push([x, y]));
                cx = cmd.args[2];
                cy = cmd.args[3];
                break;
            case "bezierCurveTo":
                sampleCubic(cx, cy, cmd.args[0], cmd.args[1], cmd.args[2], cmd.args[3], cmd.args[4], cmd.args[5], (x, y) => ring.push([x, y]));
                cx = cmd.args[4];
                cy = cmd.args[5];
                break;
            case "closePath":
...
function pathHasArea(contours) {
    for (const poly of contours) {
        for (const ring of poly) {
            if (ring.length > 2 && Math.abs(ringSignedArea(ring)) > 0.01)
                return true;
        }
    }
    return false;
...
function outlineTextNative(payload) {
    ...
    const lineHeight = Number(shape.lineHeight) > 0 ? Number(shape.lineHeight) : 1;
    ...
    for (const line of lines) {
        if (!line.text)
            continue;
        const yBaseline = line.yBaselinePaper ??
            (line.yTopPaper != null
        ...
        for (let i = 0; i < run.glyphs.length; i++) {
            ...
            const gy = yBaseline;
            const transformed = glyph.path.transform(fontScale, 0, 0, -fontScale, gx, gy);
...
function wrapParagraphFontkit(font, text, fontSize, maxWidth) {
    ...
    while (start < text.length) {
        ...
        while (end <= text.length) {
            ...
            if (lineWidthFontkit(font, slice, fontSize) > maxWidth &&
                end > start + 1) {
                end -= 1;
                break;
            }
