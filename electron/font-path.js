...
exports.getFontSearchDirs = getFontSearchDirs;
exports.findFontFile = findFontFile;
exports.findBundledFontFile = findBundledFontFile;
exports.readFontFile = readFontFile;
// CommonJS / ES module（Node のみ）。require は builtin-fonts の global script
// スコープと識別子が衝突しないよう、本ファイルは module（export を持つ）として
// 隔離する。分割代入は module-local バインドになる。
const fs = require("fs");
const os = require("os");
...
function findFontFile(family, style = "Regular", opts = {}) {
    ...
    const wantsUnicode = /arialunicode|arialunicodems/.test(needle) ||
    ...
    function collectDir(dir) {
        ...
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            const base = entry.name
                ...
                .replace(/[\s-_]/g, "")
                .replace(/\.(ttf|otf|ttc)$/, "");
            const matchesFamily = base.startsWith(needle) ||
            ...
            if (wantsUnicode && /arialunicode/.test(base))
                score += 5;
            if (/arialunicode/.test(base))
                score += 2;
            if (entry.name.toLowerCase().endsWith(".ttc"))
                score += 1;
            candidates.push({ full, score });
        }
    }
    for (const dir of getFontSearchDirs())
        collectDir(dir);
