...
exports.createHarfBuzzTextEngine = createHarfBuzzTextEngine;
exports.openHbFont = openHbFont;
// Node の require とブラウザ esbuild の ESM import 両方から使われる。
// ES module（export）として隔離し、packages/*.ts の global script 名との衝突を避ける。
const { textEngineExpandFontCandidates, textEngineAlignedX, textEngineHbPathToContoursReal, textEngineNeedsCjk, textEngineSplitScriptRuns, } = require("../packages/text-engine-utils");
const { BUILTIN_FONT_GEN, isBuiltinFontFamily, textEnginePrimaryFontFamily, } = require("../packages/builtin-fonts");
function createFontResolver(loadFontEntry) {
    const cache = new Map();
    async function resolveFontEntry(shape, fontCandidates) {
    ...
    async function resolveCjkEntry(shape, fontCandidates) {
        const bold = shape.fontWeight === "bold";
        const styles = bold
            ? ["Bold", "bold", "700"]
            : ["Regular", "Normal", "Book"];
...
function createHarfBuzzTextEngine(hb, loadFontEntry) {
    ...
    function cjkShapingEntry(primary, fallback, shape) {
        const family = textEnginePrimaryFontFamily(shape);
        if (isBuiltinFontFamily(family))
            return primary;
        return fallback || primary;
    }
    function shapeLineWithFallback(primary, cjk, text, fontSize, features, shape) {
    ...
    function measureLineWidthPaper(primary, cjk, text, fontSize, features, shape) {
        if (!text)
            return 0;
        const shaped = shapeLineWithFallback(primary, cjk, text, fontSize, features, shape);
        let w = 0;
    ...
    function wrapParagraph(primary, cjk, text, fontSize, maxWidthPaper, features, shape) {
        if (!text)
            return [""];
        ...
        while (start < text.length) {
            ...
            while (end <= text.length) {
                ...
                const w = measureLineWidthPaper(primary, cjk, slice, fontSize, features, shape);
    ...
    async function computeTextLayout(payload) {
        ...
        const lineHeightMult = Number(shape.lineHeight) > 0 ? Number(shape.lineHeight) : 1;
        ...
        for (const para of paragraphs) {
            const wrapped = wrapParagraph(primary, cjk, para, fontSize, paperWidth, features, shape);
        ...
        for (let idx = 0; idx < visualLines.length; idx++) {
            ...
            const lw = measureLineWidthPaper(primary, cjk, vLine.text, fontSize, features, shape);
    ...
    async function outlineText(payload) {
        ...
        for (const line of layout.lines) {
            if (!line.text)
                continue;
            const yBaseline = line.yTopPaper != null
                ? line.yTopPaper + ascenderPaper
                : anchorPaper.y +
                    ascenderPaper +
                    (line.lineIndex || 0) * fontSize * lineHeightMult;
            const shaped = shapeLineWithFallback(primary, cjk, line.text, fontSize, features, shape);
            let cursorPaper = 0;
...
function openHbFont(hb, buffer, faceIndex = 0) {
