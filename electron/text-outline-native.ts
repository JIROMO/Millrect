...
function textNeedsCjk(text: any): boolean {
  return /[\u3040-\u30ff\u4e00-\u9fff\u3400-\u4dbf]/.test(text || "");
}

function expandFontFamilyCandidates(shape: any): string[] {
  const raw = (shape.fontFamily || "Helvetica,Arial,sans-serif")
    .split(",")
    .map((f: string) => f.trim().replace(/^['"]|['"]$/g, ""))
    .filter((f: string) => f && !/^(sans-serif|serif|monospace)$/i.test(f));
...
function openFontkitFile(filePath: string, familyHint?: string): any {
  ...
  const match = handle.fonts.find((f: any) => {
...
function openFontkitBuffer(buffer: any, familyHint?: string): any {
...
function effectiveNativeFontFamily(shape: any): string {
  return normalizeTextFontFamily(shape?.fontFamily);
}

function resolveFontFromPayload(shape: any, payload: any): any {
  const wantBold = shape.fontWeight === "bold";
  const projectFiles = [...(payload?.projectFontFiles || [])].sort(
    (a: any, b: any) => {
...
function resolveFont(shape: any): any {
  const primaryStyle = shape.fontWeight === "bold" ? "Bold" : "Regular";
  const styles = [primaryStyle, "Regular", "Normal", "Book"];
...
function glyphToContours(glyphPath: any, scale: any): any {
  if (!glyphPath?.commands?.length) return null;
  const rings = fontkitPathToPaperRings(glyphPath.commands);
  if (!rings.length) return null;
  const contours = textEngineGroupRingsIntoPolygons(rings).map((poly: any) =>
    poly.map((ring: any) => ring.map(([x, y]: number[]) => paperToReal(x, y, scale))),
  );
...
function fontkitPathToPaperRings(commands: any): number[][][] {
  const rings: number[][][] = [];
  let ring: number[][] = [];
  let cx = 0;
  let cy = 0;
...
function paperToReal(x: number, y: number, scale: any): [number, number] {
  scale = scale || { numerator: 1, denominator: 1 };
  const f = 10 * (scale.denominator / scale.numerator);
  return [x * f, y * f];
}

function pathHasArea(contours: any): boolean {
...
function outlineTextNative(payload: any): { children: any[] } {
...
function lineWidthFontkit(font: any, text: any, fontSize: number): number {
  const fontScale = fontSize / font.unitsPerEm;
...
function measureTextLayoutNative(payload: any): any {
...
export { outlineTextNative, measureTextLayoutNative };
