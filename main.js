...
function getWsToken() {
    if (!_wsToken)
        _wsToken = crypto.randomBytes(32).toString("hex");
    return _wsToken;
}
...
function startWebSocketServer(port = WS_PORT) {
    ...
    server.once("listening", () => {
        ...
        if (port !== WS_PORT)
            console.log(`[MCP-WS] listening on port ${port}`);
    });
    server.on("connection", (ws) => {
        ws.on("message", async (data) => {
          ...
          `);
                    ...
                    const input = JSON.stringify(msg.input ?? { statement: msg.statement });
                    result = await _win.webContents.executeJavaScript(`
            (async () => {
              try {
          ...
          `);
            ...
            `);
                    ...
                    if (result === undefined) {
                        ...
                        const relPath = opts.path || `docs/images/mcp-capture-${Date.now()}.png`;
...
function buildMenu(locale) {
    if (locale)
        _appLocale = normalizeAppLocale(locale);
...
app.whenReady().then(() => {
    ...
    session.defaultSession.setPermissionCheckHandler((_webContents, permission) => permission === "local-fonts");
...
ipcMain.handle("fontLibrary:read", async () => _readJsonFileSafe(_fontLibraryPath()));
ipcMain.handle("fontLibrary:write", async (_, data) => {
    fs.writeFileSync(_fontLibraryPath(), JSON.stringify(data, null, 2));
});
ipcMain.handle("fontCatalog:read", async () => _readJsonFileSafe(_fontCatalogCachePath()));
