...
function normalizeAppLocale(code: any) {
  const c = String(code || "")
    .trim()
    .toLowerCase()
    .split("-")[0];
...
function writeWsAuthFiles(port: number) {
  const dir = wsAuthDir();
...
function sendOpenHelpDoc(page: any, anchor?: any) {
...
app.whenReady().then(() => {
  ...
  session.defaultSession.setPermissionCheckHandler(
    (_webContents: any, permission: any) => permission === "local-fonts",
  );
...
ipcMain.handle("help:openTopic", async (_: any, { page, anchor }: any) => {
  sendOpenHelpDoc(page, anchor);
  return { ok: true };
});

ipcMain.handle("app:setLocale", async (_: any, locale: any) => {
  buildMenu(locale);
  return { ok: true, locale: _appLocale };
});

async function saveProjectJsonDialog(_: any, { defaultName, content }: any) {
...
ipcMain.handle("dialog:saveSvg", async (_: any, { defaultName, content }: any) => {
  const { filePath } = await dialog.showSaveDialog({
...
ipcMain.handle("dialog:savePdf", async (_: any, { defaultName, buffer }: any) => {
  const { filePath } = await dialog.showSaveDialog({
    defaultPath: path.join(app.getPath("downloads"), defaultName),
...
function _readJsonFileSafe(filePath: string) {
  try {
...
ipcMain.handle("fontLibrary:write", async (_: any, data: any) => {
...
ipcMain.handle("font:measureTextLayout", async (_: any, payload: any) => {
  try {
...
// このファイルは ES module（module-local スコープ）として扱う。トップレベルの
// `const crypto = require("crypto")` 等が @types/node の global と衝突するのを防ぐ。
export {};
